...
export class TaskRepository implements ITaskRepository {
  ...
  async createTask(taskData: {
    ...
    if (!taskData.creator)
      throw new NotFoundException('Creator ID is required');
    ...
    const createdBy = await this.userRepository.getCreatorById(
      taskData.creator,
    );
    const department = await this.departmentRepo.findOne({
      where: { id: taskData.department },
    });

    if (!department) throw new NotFoundException('Department not found');

    const assignedUsers = await this.userRepository.validateAssignedUsers(
